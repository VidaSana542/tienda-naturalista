...
function stockLabel(stock) {
    if (stock === 'in') return '<span class="stock-badge stock-in"><svg viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>Disponible</span>';
    if (stock === 'low') return '<span class="stock-badge stock-low"><svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>Pocas unidades</span>';
    return '<span class="stock-badge stock-out"><svg viewBox="0 0 24 24"><path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"/></svg>Agotado</span>';
}
...
function renderProducts() {
    ...
    function cardHtml(p) {
        ...
        const cartDisabled = disabled ? 'disabled style="opacity:0.4;cursor:not-allowed;"' : '';
        const cartClick = disabled ? '' : `addToCart(${p.id})`;
        const waClick = disabled ? '' : `addToCart(${p.id});setTimeout(()=>openCheckout(),100)`;
        return `
        <div class="prod-card">
            ${badgeHtml}
            ...
            <div class="prod-body">
                ...
                <div class="prod-actions">
                    ...
                    <button class="prod-btn-cart" onclick="${cartClick}" ${cartDisabled}>
...
function renderCartPanel() {
    ...
    container.innerHTML = cart.map(c => `
        <div class="cart-item">
            ...
            <div class="cart-item-info">
                ...
                <div class="cart-item-sub">Subtotal: ${formatPrice(c.price * c.qty)}</div>
    ...
    subtotals.innerHTML = `
        <div><span>Subtotal (${items} items)</span><span>${formatPrice(subtotal)}</span></div>
    `;
