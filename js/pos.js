...
function renderDashboard() {
    ...
    document.getElementById('dashStats').innerHTML = `
        <div class="stat-card"><div class="stat-icon green"><svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg></div><div class="stat-info"><span class="stat-label">VENTAS</span><h3>${formatPrice(periodTotal)}</h3><p>${periodCount} ventas</p></div></div>
...
function saveProduct() {
    ...
    if (id) {
        ...
        if (p) {
            ...
            Object.assign(p, { name, catalogName, barcode, brand, category, price, cost, stock, img: finalImg, images, desc, supplier, featured, subcategory });
            if (stock !== prevStock) {
        ...
        posProducts.push({ id: 'p' + posNextProductId++, name, catalogName, barcode, brand, category, price, cost, stock, img: finalImg, images, desc, supplier, featured, subcategory, _synced: false });
    }
    saveProducts();
    closeProductModal();
    renderProductTable();
